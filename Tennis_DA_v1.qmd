...
Number of result 1 and 2 are very close to each other. So, data is balanced.
...
As seen on the schema, output weights and biases are very close to each other. Therefore, variable weights can be related to importance of variable.
...
Model comparison table show that Random Forest model has the best accuracy. Also when we consider importance of variable, all of the model indicate that FSW, BPC, SSW are the most effective statistics that effect the result. 
Therefore, players should practice serving better and returning serves from opponent in order to win the match.
...
As a conclusion, Grand slams 2013 data set analyzed by considering Result and total points win. For predicting result, classification modeling applied as logistic regression, svm, random forest and ANN. Random forest show the best performance for predicting match result. Also, according to all classification model FSW and BPC are the most effective variables.
Moreover, TPW regression made by classical linear model, lasso and ridge. All of the regression model show that again FSW and BPC are most important variables for TPW.
Therefore, players should focus on serving and returning oppenent services to win the match and win more points.